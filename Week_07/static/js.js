...
document.querySelector('#addscore a').addEventListener('click', function() {
    ...
    if (entry[0] == '') {
        document.querySelector('#addscore .alert').innerHTML = 'ENTER NAME!'
    }
    
    else if (entry[1].search(/[0-9]{4}–[0-9]{2}–[0-9]{2}/)== -1) {
        document.querySelector('#addscore .alert').innerHTML = 'INVALID DATE!'
    }
    
    else if (entry[2].search(/^[0-9]*$/)< 0) {
        document.querySelector('#addscore .alert').innerHTML = 'INVALID SCORE!'
    }
